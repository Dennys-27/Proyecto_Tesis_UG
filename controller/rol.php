...
switch ($_GET["op"]) {
   ...
   case "listar":
    ...
    foreach ($datos as $row) {
        ...
        // Botones
        $sub_array[] = '
            <button type="button" onClick="permisos(' . $row["id_rol"] . ')" class="btn-icon btn-primary">
                <svg viewBox="0 0 24 24"><path d="M5 20h14v-2H5v2zm7-18l-5 5h3v4h4V7h3l-5-5z"/></svg>
            </button>';

        $sub_array[] = '
            <button type="button" onClick="editar(' . $row["id_rol"] . ')" class="btn-icon btn-warning">
        ...
        $sub_array[] = '
            <button type="button" onClick="eliminar(' . $row["id_rol"] . ')" class="btn-icon btn-danger">
                <svg viewBox="0 0 24 24"><path d="M6 19c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V7H6v12zM19 4h-3.5l-1-1h-5l-1 1H5v2h14V4z"/></svg>
            </button>';

        $sub_array[] = '
            <button type="button" onClick="activar(' . $row["id_rol"] . ')" class="btn-icon btn-primary">
    ...
    case "eliminar":
        $datos = $rol->eliminar_rol($_POST["rol_id"]);      
        echo json_encode($datos);
        break;
    
    case "activar":
        $datos = $rol->activar_rol($_POST["rol_id"]);      
        echo json_encode($datos);
        break;

    case "combo":
        $datos = $rol->get_rol_combo();
        echo json_encode($datos);
        break;
